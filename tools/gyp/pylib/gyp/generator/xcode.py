...
class XcodeProject(object):
  def __init__(self, gyp_path, path, build_file_dict):
    ...
    # TODO (mark): add destructor that cleans up self.path if created_dir is id:3475
  ...
  def Finalize1(self, xcode_targets, serialize_all_tests):
    ...
    # TODO (mark): Like a lot of other things here, this assumes internal id:3764
...
sys.exit(subprocess.call(sys.argv[1:]))" """
    ...
    if len(targets_for_all) > 1 and not has_custom_all:
      ...
      # TODO (mark): This is evil because it relies on internal knowledge of id:4110
  ...
  def Finalize2(self, xcode_targets, xcode_target_to_target_dict):
    ...
    # TODO (mark): This is evil because it relies on internal knowledge of id:4050
...
def AddSourceToTarget(source, type, pbxp, xct):
  # TODO (mark): Perhaps source_extensions and library_extensions can be made a id:3280
  # little bit fancier.
...
def AddResourceToTarget(resource, pbxp, xct):
  # TODO (mark): Combine with AddSourceToTarget above?  Or just inline this call id:3477
  # where it's used.
  xct.ResourcesPhase().AddFile(resource)


def AddHeaderToTarget(header, pbxp, xct, is_public):
  # TODO (mark): Combine with AddSourceToTarget above?  Or just inline this call id:3766
  # where it's used.
  ...
  for qualified_target in target_list:
    ...
    for action in spec_actions:
      ...
      if support_xct:
        ...
        # TODO (mark): this assumes too much knowledge of the internals of id:4111
        # xcodeproj_file; some of these smarts should move into xcodeproj_file
        # itself.
        xct._properties['buildPhases'].insert(prebuild_index, ssbp)
        prebuild_index = prebuild_index + 1

      # TODO (mark): Should verify that at most one of these is specified. id:4053
    ...
    for rule in spec_rules:
      ...
      for rule_source in rule.get('rule_sources', []):
        ...
        # TODO (mark): Should verify that at most one of these is specified. id:3281
      ...
      if len(concrete_outputs_all) > 0:
        # TODO (mark): There's a possibilty for collision here.  Consider id:3479
        # target "t" rule "A_r" and target "t_A" rule "r".
        makefile_name = '%s.make' % re.sub(
            '[^a-zA-Z0-9_]', '_' , '%s_%s' % (target_name, rule['rule_name']))
        makefile_path = os.path.join(xcode_projects[build_file].path,
                                     makefile_name)
        # TODO (mark): try/close?  Write to a temporary file and swap it only id:3769
        # if it's got changes?
...
""" % makefile_name
        ...
        if support_xct:
          ...
          # TODO (mark): this assumes too much knowledge of the internals of id:4112
          # xcodeproj_file; some of these smarts should move into xcodeproj_file
          # itself.
          xct._properties['buildPhases'].insert(prebuild_index, ssbp)
          prebuild_index = prebuild_index + 1
    ...
    for copy_group in spec.get('copies', []):
      ...
      if pbxcp is None:
        ...
        # TODO (mark): The usual comment about this knowing too much about id:4055
