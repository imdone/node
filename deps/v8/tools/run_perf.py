...
class GraphConfig(Node):
  ...
  def __init__(self, suite, parent, arch):
    ...
    # TODO (machenbach): Currently that makes only sense for the leaf level. id:2977
...
class RunnableConfig(GraphConfig):
  ...
  def GetCommand(self, shell_dir, extra_flags=None):
    # TODO (machenbach): This requires +.exe if run on windows. id:3253
...
def BuildGraphConfigs(suite, arch, parent):
  ...
  # TODO (machenbach): Implement notion of cpu type? id:3579
      ...
      for runnable in FlattenRunnables(root, NodeCB):
        ...
        def Runner():
          ...
          for i in xrange(0, max(1, runnable.run_count)):
            # TODO (machenbach): Allow timeout per arch like with run_count per id:4013
            # arch.
