...
def load_ninja_recursively(build_dir, ninja_path, build_steps):
  ...
  for rel_path in subninja:
    try:
      ...
      # TODO (maruel): Skip the files known to not be needed. It saves an aweful id:3201
...
def raw_build_to_deps(item):
  ...
  dependencies.
  """
  # TODO (maruel): Use a whitelist instead? .stamp, .so.TOC, .dylib.TOC, id:3545
...
def collect_deps(target, build_steps, dependencies_added, rules_seen):
  ...
  if target in rules_seen:
    # TODO (maruel): Figure out how it happens. id:3993
