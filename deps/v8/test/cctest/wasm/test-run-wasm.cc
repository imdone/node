...
WASM_EXEC_TEST(LoadMaxUint32Offset) {
  // TODO (eholk): Fix this test for the trap handler. id:2456
...
WASM_EXEC_TEST(LoadMemI32_oob) {
  // TODO (eholk): Fix this test for the trap handler. id:2657
  if (trap_handler::UseTrapHandler()) return;
...
WASM_EXEC_TEST(LoadMem_offset_oob) {
  // TODO (eholk): Fix this test for the trap handler. id:2764
  if (trap_handler::UseTrapHandler()) return;
...
WASM_EXEC_TEST(LoadMemI32_const_oob_misaligned) {
  // TODO (eholk): Fix this test for the trap handler. id:3686
  if (trap_handler::UseTrapHandler()) return;
  constexpr byte kMemSize = 12;
  // TODO (titzer): Fix misaligned accesses on MIPS and re-enable. id:3418
...
WASM_EXEC_TEST(LoadMemI32_const_oob) {
  // TODO (eholk): Fix this test for the trap handler. id:2458
  if (trap_handler::UseTrapHandler()) return;
...
WASM_EXEC_TEST(StoreMem_offset_oob) {
  // TODO (eholk): Fix this test for the trap handler. id:2658
  if (trap_handler::UseTrapHandler()) return;
...
WASM_EXEC_TEST(MultiReturnSelect_i64) {
  ...
  // TODO (titzer): implement int64-lowering for multiple return values id:2765
