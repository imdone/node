...
static void VisitBinop(InstructionSelector* selector, Node* node,
  ...
  // TODO (turbofan): match complex addressing modes. id:1900
  if (left == right) {
...
// TODO (bmeurer): Merge this with VisitWord64Shift using template magic? id:1543
...
// TODO (bmeurer): Merge this with VisitWord32Shift using template magic? id:2916
...
void VisitMulHigh(InstructionSelector* selector, Node* node,
  ...
  // TODO (turbofan): We use UseUniqueRegister here to improve register id:1972
  // allocation.
...
void InstructionSelector::VisitInt32MulWithOverflow(Node* node) {
  // TODO (mvstanton): Use Int32ScaleMatcher somehow. id:2051
  if (Node* ovf = NodeProperties::FindProjection(node, 1)) {
    FlagsContinuation cont = FlagsContinuation::ForSet(kOverflow, ovf);
  ...
  if (descriptor->IsCFunctionCall()) {
    ...
    for (PushParameter input : base::Reversed(*arguments)) {
      ...
      if (g.CanBeImmediate(input_node)) {
        ...
        // TODO (titzer): X64Push cannot handle stack->stack double moves id:1903
...
InstructionCode TryNarrowOpcodeSize(InstructionCode opcode, Node* left,
  ...
  // TODO (epertoso): we can probably get some size information out phi nodes. id:1546
