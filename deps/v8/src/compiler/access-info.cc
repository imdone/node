...
bool CanInlinePropertyAccess(Handle<Map> map) {
  ...
  return map->IsJSObjectMap() && !map->is_dictionary_map() &&
         ...
         // TODO (verwaest): Whitelist contexts to which we have access. id:1762
      ...
      if (details.location() == kField) {
        if (details.kind() == kData) {
          ...
          *access_info = PropertyAccessInfo::DataField(
          ...
          // TODO (turbofan): Add support for general accessors? id:1630
          return false;
        }
...
bool AccessInfoFactory::LookupTransition(Handle<Map> map, Handle<Name> name,
  ...
  if (TransitionArray::SearchTransition(map, kData, name, NONE)
    ...
    // TODO (bmeurer): Handle transition to data constant? id:1235
