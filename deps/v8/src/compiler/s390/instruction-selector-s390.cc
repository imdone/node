...
#undef VISITOR
    return true;
    // TODO (john.yan): consider the following case to be valid id:2003
...
// TODO (john.yan): Create VisiteShift to match dst = src shift (R+I) id:1863
#if 0
void VisitShift() { }
#endif
...
void InstructionSelector::VisitWord64Shl(Node* node) {
  S390OperandGenerator g(this);
  Int64BinopMatcher m(node);
  // TODO (mbrandy): eliminate left sign extension if right >= 32 id:1922
  ...
  if (CanCover(node, input.node()) && input.IsLoad()) {
    ...
    if (load_rep.representation() == MachineRepresentation::kWord32) {
      ...
      Emit(kS390_LoadReverse32 | AddressingModeField::encode(kMode_MRR),
           // TODO (john.yan): one of the base and offset can be imm. id:2004
...
// TODO (john.yan): place kAllowRM where available id:1864
#define FLOAT_UNARY_OP_LIST_32(V)                                              \
  ...
  /* TODO (john.yan): can use kAllowRM  id:1425*/                                       \
...
void VisitLoadAndTest(InstructionSelector* selector, InstructionCode opcode,
  ...
  // TODO (john.yan): Add support for Float32/Float64. id:2878
