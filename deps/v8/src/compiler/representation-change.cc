...
//                   kNone
//
// TODO (jarin) We might consider making kBool < kFloat64. id:1914
...
Node* RepresentationChanger::GetTaggedSignedRepresentationFor(
  ...
  if (output_type->Is(Type::None())) {
    ...
    if (use_info.type_check() == TypeCheckKind::kSignedSmall) {
      // TODO (turbofan): Consider adding a Bailout operator that just deopts. id:1996
...
Node* RepresentationChanger::GetTaggedPointerRepresentationFor(
  ...
  if (output_type->Is(Type::None())) {
    ...
    // TODO (turbofan): Consider adding a Bailout operator that just deopts id:1860
...
Node* RepresentationChanger::GetFloat64RepresentationFor(
  ...
  // Eagerly fold representation changes for constants.
  if ((use_info.type_check() == TypeCheckKind::kNone)) {
    // TODO (jarin) Handle checked constant conversions. id:1412
    switch (node->opcode()) {
      case IrOpcode::kNumberConstant:
  ...
  if (output_type->Is(Type::None())) {
    ...
    if (output_type->Is(Type::Undefined())) {
      ...
      // TODO (jarin) Here we should check that truncation is Number. id:2870
      op = simplified()->TruncateTaggedToFloat64();
