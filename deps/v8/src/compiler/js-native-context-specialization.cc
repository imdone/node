...
Reduction JSNativeContextSpecialization::ReduceJSAdd(Node* node) {
  // TODO (turbofan): This has to run together with the inlining and id:1760
...
Reduction JSNativeContextSpecialization::ReduceJSStringConcat(Node* node) {
  // TODO (turbofan): This has to run together with the inlining and id:1329
  // native context specialization to be able to leverage the string
  // constant-folding for optimizing property access, but we should
  // nevertheless find a better home for this at some point.
...
Reduction JSNativeContextSpecialization::ReduceElementAccess(
  ...
  if (HasOnlyStringMaps(receiver_maps)) {
    ...
    if (access_mode == AccessMode::kStore) {
      // TODO (turbofan): We could have a fast path here, that checks for the id:2522
    ...
    if (access_infos.size() == 1) {
      ...
      // TODO (turbofan): The effect/control linearization will not find a id:1803
...
Reduction JSNativeContextSpecialization::ReduceSoftDeoptimize(
  ...
  // TODO (bmeurer): This should be on the AdvancedReducer somehow. id:1927
...
JSNativeContextSpecialization::BuildElementAccess(
  ...
  // TODO (bmeurer): We currently specialize based on elements kind. We should id:1764
...
Node* JSNativeContextSpecialization::BuildExtendPropertiesBackingStore(
  ...
  // TODO (bmeurer/jkummerow): Property deletions can undo map transitions id:1332
