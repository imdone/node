...
// TODO (dcarney): fix frame to allow frame accesses to half size location. id:1858
  ...
  for (int index = block->last_instruction_index(); index >= block_start;
    ...
    for (size_t i = 0; i < instr->OutputCount(); i++) {
      ...
      if (block->IsHandler() && index == block_start && output->IsAllocated() &&
        ...
        // exception value.
        // TODO (mtrofin): should we explore an explicit opcode for id:1406
...
void LiveRangeBuilder::ProcessPhis(const InstructionBlock* block,
  ...
  for (PhiInstruction* phi : block->phis()) {
    ...
    for (RpoNumber predecessor : block->predecessors()) {
      ...
      // called.
      // TODO (v8): Find a way to separate hinting from live range analysis in id:2605
...
void LiveRangeBuilder::BuildLiveRanges() {
  ...
  for (TopLevelLiveRange* range : data()->live_ranges()) {
    ...
    // TODO (bmeurer): This is a horrible hack to make sure that for constant id:1913
...
void LinearScanAllocator::FindFreeRegistersForRange(
  ...
  for (LiveRange* cur_inactive : inactive_live_ranges()) {
    ...
    // TODO (mtrofin): extend to aliased ranges, too. id:1994
    if ((kSimpleFPAliasing || !check_fp_aliasing()) &&
...
void LinearScanAllocator::AllocateBlockedReg(LiveRange* current) {
  ...
  for (LiveRange* range : inactive_live_ranges()) {
    ...
    // TODO (mtrofin): extend to aliased ranges, too. id:1859
    if ((kSimpleFPAliasing || !check_fp_aliasing())) {
      if (is_fixed) {
