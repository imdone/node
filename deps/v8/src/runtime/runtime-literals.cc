...
struct ObjectBoilerplate {
  static Handle<JSObject> Create(Isolate* isolate,
    ...
    // TODO (verwaest): Support tracking representations in the boilerplate. id:2544
    ...
    if (map->is_dictionary_map() && !has_null_prototype) {
      // TODO (cbruni): avoid making the boilerplate fast again, the clone stub id:3009
...
MaybeHandle<JSObject> CreateLiteral(Isolate* isolate,
  ...
  if (HasBoilerplate(isolate, literal_site)) {
    ...
    // TODO (cbruni): Even in the case where we need an initial allocation site id:3230
