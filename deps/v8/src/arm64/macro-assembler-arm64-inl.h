...
void TurboAssembler::Fmov(VRegister vd, double imm) {
  ...
  if (IsImmFP64(imm)) {
    ...
    if (vd.IsScalar()) {
      ...
      // TODO (all): consider NEON support for load literal. id:1495
      Movi(vd, bits);
    }
  }
}
  ...
  if (IsImmFP32(imm)) {
    ...
    if (vd.IsScalar()) {
      if (bits == 0) {
        ...
        // TODO (all): Use Assembler::ldr(const VRegister& ft, float imm). id:1059
        Mov(tmp, bit_cast<uint32_t>(imm));
        Fmov(vd, tmp);
      }
    } else {
      // TODO (all): consider NEON support for load literal. id:1018
      Movi(vd, bits);
    }
  }
}
...
void TurboAssembler::BumpSystemStackPointer(const Operand& space) {
  ...
  if (!TmpList()->IsEmpty()) {
    ...
    // TODO (jbramley): Several callers rely on this not using scratch id:1639
