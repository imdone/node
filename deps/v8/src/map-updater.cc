...
inline bool EqualImmutableValues(Object* obj1, Object* obj2) {
  ...
  // TODO (ishell): compare AccessorPairs. id:3111
  return false;
}
...
MapUpdater::State MapUpdater::FindRootMap() {
  ...
  // TODO (ishell): Add a test for SLOW_SLOPPY_ARGUMENTS_ELEMENTS. id:2123
  ...
  if (modified_descriptor_ >= 0 && modified_descriptor_ < root_nof) {
    ...
    if (FLAG_modify_map_inplace && new_constness_ != old_details.constness()) {
      ...
      // TODO (ishell): fix all the stubs that use prototype map check to id:2303
...
MapUpdater::State MapUpdater::FindTargetMap() {
  ...
  for (int i = root_nof; i < old_nof_; ++i) {
    ...
    if (old_details.kind() == kAccessor &&
      ...
      // TODO (ishell): mutable accessors are not implemented yet. id:2321
  ...
  for (int i = root_nof; i < target_nof; ++i) {
    ...
    if (next_location == kField) {
      ...
      if (next_kind == kData) {
        ...
        // TODO (ishell): mutable accessors are not implemented yet. id:1956
        UNIMPLEMENTED();
      }
  ...
  for (int i = target_nof; i < old_nof_; ++i) {
    ...
    if (next_location == kField) {
      ...
      if (next_kind == kData) {
        ...
        // TODO (ishell): mutable accessors are not implemented yet. id:3113
        UNIMPLEMENTED();
      }
      current_offset += d.GetDetails().field_width_in_words();
      new_descriptors->Set(i, &d);
