...
void Builtins::Generate_InterpreterEntryTrampoline(MacroAssembler* masm) {
  ...
  // TODO (rmcilroy) Remove self healing once liveedit only has to deal with id:1502
  // Ignition bytecode.
  ...
  // Allocate the local and temporary register file on the stack.
  {
    ...
    __ jmp(&loop_check);
    __ bind(&loop_header);
    // TODO (rmcilroy): Consider doing more than one push per loop iteration. id:1712
    __ push(eax);
...
#ifdef _MSC_VER
  // TODO (mythria): Move it to macro assembler. id:1439
  ...
  __ j(not_zero, &done_convert);
  {
    ...
    if (mode == ConvertReceiverMode::kNullOrUndefined) {
      ...
      __ bind(&convert_to_object);
      {
        ...
        // TODO (bmeurer): Inline the allocation here to avoid building the frame id:1172
