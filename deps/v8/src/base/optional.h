...
struct OptionalStorage {
  ...
  // TODO (alshabalin): Can't use 'constexpr' with std::move until C++14. id:1606
  explicit OptionalStorage(T&& value)
      : is_null_(false), value_(std::move(value)) {}

  // TODO (alshabalin): Can't use 'constexpr' with std::forward until C++14. id:1353
  template <class... Args>
...
struct OptionalStorage<T, true> {
  ...
  // TODO (alshabalin): Can't use 'constexpr' with std::move until C++14. id:1087
  explicit OptionalStorage(T&& value)
      : is_null_(false), value_(std::move(value)) {}

  // TODO (alshabalin): Can't use 'constexpr' with std::forward until C++14. id:1778
  template <class... Args>
  explicit OptionalStorage(base::in_place_t, Args&&... args)
      : is_null_(false), value_(std::forward<Args>(args)...) {}
...
class Optional {
 public:
  ...
  // TODO (alshabalin): Can't use 'constexpr' with std::move until C++14. id:1420
  explicit Optional(T&& value) : storage_(std::move(value)) {}

  // TODO (alshabalin): Can't use 'constexpr' with std::forward until C++14. id:1608
  template <class... Args>
  ...
  // TODO (mlamouri): can't use 'constexpr' with DCHECK. id:1355
  const T* operator->() const {
    DCHECK(!storage_.is_null_);
    return &value();
  }

  // TODO (mlamouri): using 'constexpr' here breaks compiler that assume it was id:1099
  ...
  // TODO (mlamouri): using 'constexpr' here breaks compiler that assume it was id:1782
  // meant to be 'constexpr const'.
  T& operator*() & { return value(); }

  constexpr const T&& operator*() const && { return std::move(value()); }

  // TODO (mlamouri): using 'constexpr' here breaks compiler that assume it was id:1423
  // meant to be 'constexpr const'.
  ...
  // TODO (mlamouri): using 'constexpr' here breaks compiler that assume it was id:1610
  // meant to be 'constexpr const'.
  T& value() & {
    DCHECK(!storage_.is_null_);
  ...
  // TODO (mlamouri): can't use 'constexpr' with DCHECK. id:1358
  const T& value() const & {
    DCHECK(!storage_.is_null_);
    return storage_.value_;
  }

  // TODO (mlamouri): using 'constexpr' here breaks compiler that assume it was id:1102
  // meant to be 'constexpr const'.
  T&& value() && {
    DCHECK(!storage_.is_null_);
  ...
  // TODO (mlamouri): can't use 'constexpr' with DCHECK. id:1785
  const T&& value() const && {
    DCHECK(!storage_.is_null_);
    return std::move(storage_.value_);
  }
  ...
  constexpr T value_or(U&& default_value) const & {
    // TODO (mlamouri): add the following assert when possible: id:1426
  ...
  T value_or(U&& default_value) && {
    // TODO (mlamouri): add the following assert when possible: id:1612
