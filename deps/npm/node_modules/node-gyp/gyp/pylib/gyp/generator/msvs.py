...
# TODO: Remove once bots are on 2.7, http://crbug.com/241769 id:702
...
# TODO (gspencer): Switch the os.environ calls to be id:410
...
def _ToolSetOrAppend(tools, tool_name, setting, value, only_if_unset=False):
  # TODO (bradnelson): ugly hack, fix this more generally!!! id:494
...
def _BuildCommandLineForRuleRaw(spec, cmd, cygwin_shell, has_input_path,
  ...
  if cygwin_shell:
    ...
    # TODO (quote):  regularize quoting path names throughout the module id:739
    cmd = ''
    if do_setup_env:
    ...
    # Fix the paths
    # TODO (quote): This is a really ugly heuristic, and will miss path fixing id:794
    ...
    if quote_cmd:
      ...
      # TODO (quote):  regularize quoting path names throughout the module id:706
...
# TODO: Avoid code duplication with _ValidateSourcesForOSX in make.py. id:412
def _ValidateSourcesForMSVSProject(spec, version):
  ...
  # TODO: Check if MSVC allows this for loadable_module targets. id:496
...
def _GenerateMSVSProject(project, options, version, generator_flags):
  ...
  # target.
  # TODO: Take excluded sources into consideration if possible. id:742
  ...
  # NOTE: this stanza must appear after all actions have been decided. id:795
...
def _GetIncludeDirs(config):
  ...
  """
  # TODO (bradnelson): include_dirs should really be flexible enough not to id:708
...
def _AdjustSourcesAndConvertToFilterHierarchy(
  ...
  # NOTE: sources goes from being a set to a list here. id:415
  # NOTE: excluded_sources goes from being a set to a list here. id:499
...
def _GetMSBuildConfigurationGlobalProperties(spec, configurations, build_file):
  # TODO (jeanluc) We could optimize out the following and do it only if id:745
  # there are actions.
  # TODO (jeanluc) Handle the equivalent of setting 'CYGWIN=nontsec'. id:797
  new_paths = []
  cygwin_dirs = spec.get('msvs_cygwin_dirs', ['.'])[0]
  if cygwin_dirs:
    cyg_path = '$(MSBuildProjectDirectory)\\%s\\bin\\' % _FixPath(cygwin_dirs)
    new_paths.append(cyg_path)
    # TODO (jeanluc) Change the convention to have both a cygwin_dir and a id:710
    # python_dir.
...
def _GetMSBuildPropertyGroup(spec, label, properties):
  ...
  # NOTE: reverse(topsort(DAG)) = topsort(reverse_edges(DAG)) id:418
...
def _FinalizeMSBuildSettings(spec, configuration):
  ...
  # Add the information to the appropriate tool
  # TODO (jeanluc) We could optimize and generate these settings only if id:502
...
def _GetValueFormattedForMSBuild(tool_name, name, value):
  if type(value) == list:
    ...
    # TODO (jeanluc) Is this what we want? id:750
...
def _GenerateMSBuildProject(project, options, version, generator_flags):
  ...
  if not spec.get('msvs_external_builder'):
    ...
    # NOTE: this stanza must appear after all actions have been decided. id:798
    # Don't excluded sources with actions attached, or they won't run.
    excluded_sources = _FilterActionsFromExcluded(
        excluded_sources, actions_to_add)
  ...
  # TODO (jeanluc) File a bug to get rid of runas.  We had in MSVS: id:713
...
def _AddMSBuildAction(spec, primary_input, inputs, outputs, cmd, description,
  ...
  action_spec.extend(
      # TODO (jeanluc) 'Document' for all or just if as_sources? id:420
      [['FileType', 'Document'],
       ['Command', command],
       ['Message', description],
       ['Outputs', outputs]
